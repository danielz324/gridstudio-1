...
if [[ "$OSTYPE" == "msys" ]]; then
	...
	if [ ! "$(docker ps -a | grep gridstudio)" ]; then
		docker run --name=gridstudio --rm=false -v $PWD/grid-app:/home/source -v $PWD/grid-app/proxy/userdata:/home/userdata -p 8090:8080 -p 4490:4430 ricklamers/gridstudio:release
	else
		echo "gridstudio container detected - starting container - want to do a full restart? Run destroy.sh first."
		docker start gridstudio
	fi
